...
int main(int arc, char **argv)
{
    ...
    if ((strcmp(algorithm, "rr") == 0) && (quantom_time == 0))
    {
        printf("You need to provide a quantom time with -q when using round robin.\n");
        return 0;
    }
    ...
    else if (strcmp(algorithm, "rr") == 0)
    {
        ...
        bool processes_done[MAX_PROCESSES] = {false};
        int time = 0;
        int processes_burst_time[MAX_PROCESSES] = {0};
        int processes_wait_time[MAX_PROCESSES] = {0};
        int processes_turnaround_time[MAX_PROCESSES] = {0};
        bool done = false;
        while (done == false)
        {
            for (int i = 0; i < process_count; i++)
            {
                ...
                processes_burst_time[i] += quantom_time;
                time += quantom_time;
                if (processes_burst_time[i] >= processes[i].burst_time)
                {
                    // printf("process is done id %d wait %d turn %d \n", processes[i].id, time - processes[i].burst_time, time + processes[i].burst_time - processes_burst_time[i]);
                    processes_done[i] = true;
                    struct Process_Result new_process = {processes[i].id, time - processes[i].burst_time, time + processes[i].burst_time - processes_burst_time[i]};
                    processes_result[i] = new_process;
                }
                // printf("one cycle id %d processes_burst_time %d  time %d\n", i, processes_burst_time[i], time);
            }
            done = true;
            for (int i = 0; i < process_count; i++)
            {
                if (processes_done[i] == false)
                {
                    done = false;
                }
            }
