...
class TerminalUI:
    ...
    def show_2d(self):
        ...
        for i, v in enumerate(self._tree.bfs_order_star()):
            if i == level_cap:
                level_cap += 2**level
                level += 1
                print("\n", " "*(int(1.5*2**(height-level))-1), end='')
            if level == height+1:
                return
            if(v==None):
                print("*",  end=' '*(3*int(2**(height-level))-1))
            else:
                print(v, end=' '*(3*int(2**(height-level))-1-(1 if v > 9 else 0)))
